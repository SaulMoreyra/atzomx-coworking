...
const ProductCard: FC<ProductCardProps> = ({
  ...
  return (
    <m.div
      ...
      initial="hidden"
      whileHover="hover"
      className="bg-white rounded-md flex flex-col justify-between overflow-hidden items-center relative p-10 w-full h-full">
      <Link href={`/product/${name}`} className="p-10 w-full z-0 relative">
        <div className="relative w-full aspect-square">
          <Image
            ...
            alt={name}
            fill={true}
            className="rounded-md transition-zoom"
          />
        </div>
      </Link>
      ...
      {showAddToCartButton && (
        <div className="flex justify-center gap-2 items-center">
          {ctaButtons.map(
            ({ icon, onClick, isLoading, tooltipId, tooltipText }, index) => (
